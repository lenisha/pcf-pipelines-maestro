  ...
  do
    ...
    if [ -z "${previousBPName}" ]; then
        ...
        if [ "${previousBPName%_*}" != "${bpName%_*}" ]; then   # check if base names of buildpack match from the two entries
            ...
            cat ./buildpack-upgrade.yml | ./yaml_patch -o ./tmpBuildpackPatch.yml > ./buildpack-upgrade-final.yml
            ...
            if [ -z "${buildpackGlobs}" ]; then
                ...
                cat ./buildpack-upgrade-final.yml | ./yaml_patch -o ./add-buildpack-globs.yml > ./buildpack-upgrade.yml
            fi
            previousBPName=""
            previousBPVersion=""
            previousBPEntryKey=""
